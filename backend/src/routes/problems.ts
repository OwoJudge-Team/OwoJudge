...
const runTpsGen = async (problem: IProblem, releaseLock: () => void) => {
  try {
    console.log(
      `Starting async test generation for ${problem.serialNumber} in isolated environment`,
    );
    ...
    await IsolateManager.withBox(async (box) => {
      ...
      try {
        ...
        const finalProblemDir = "problems/" + String(problem.serialNumber);
        ...
        if (fs.existsSync(generatedTestsDir)) {
          ...
          try {
            ...
            console.log(
              `Test cases generated and copied successfully for ${problem.serialNumber}`,
            );
    ...
    console.error(
      `Failed to generate test cases for ${problem.serialNumber}:`,
      genError,
    );
    await Problem.findByIdAndUpdate(problem._id, {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        runTpsGen(newProblem, releaseLock);
      } catch (error) {
        releaseLock();
        throw error;
      }
