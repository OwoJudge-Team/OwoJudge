...
/**
 * Runs `tps gen` in an isolated sandbox, copies the generated tests back, validates
 * that on-demand generation works via {@link generateSingleTestcase}, then updates
 * the problem status to `ready` (or `error` with a reason on failure).
 *
 * Intended to be called inside a fire-and-forget async IIFE after the problem
 * directory has been placed at `finalProblemDir`.  The caller must pass the
 * `releaseLock` callback so the ProblemLock is always released.
 */
const runTestGeneration = async (
  problemMongoId: string,
  serialNumber: number,
  finalProblemDir: string,
  releaseLock: () => void,
): Promise<void> => {
  try {
    console.log(
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        // Generate test cases asynchronously — fire and forget, lock released inside helper
        runTestGeneration(
          newProblem._id.toString(),
          newProblem.serialNumber,
          finalProblemDir,
          releaseLock,
        );
      } catch (error) {
