...
if [ "${LANG}" == "cpp" ] ; then
	variable_exists "CPP_STD_OPT" || CPP_STD_OPT="--std=c++20"
	...
	variable_exists "C_STD_OPT" || C_STD_OPT="-std=c17"
	vecho "C_STD_OPT='${C_STD_OPT}'"
	variable_exists "C_WARNING_OPTS" || C_WARNING_OPTS="-Wall -Wextra -Wshadow"
	vecho "C_WARNING_OPTS='${C_WARNING_OPTS}'"
	variable_exists "C_OPTS" || C_OPTS="-DEVAL ${C_STD_OPT} ${C_WARNING_OPTS} -O2"
	vecho "C_OPTS='${C_OPTS}'"
	set_coloring_flag "-fdiagnostics-color=always" "-fdiagnostics-color=never"
	files_to_compile=("${prog}")
	if is_windows; then
	...
	if "${HAS_GRADER}"; then
		grader_header="${PROBLEM_NAME}.h"
		grader_c="${GRADER_NAME}.c"
		vecho "Copying '${grader_header}' and '${grader_c}' to sandbox..."
		vrun cp "${GRADER_LANG_DIR}/${grader_header}" "${GRADER_LANG_DIR}/${grader_c}" "."
		vecho "Compiling grader..."
		vrun capture_compile gcc ${C_OPTS} -c "${grader_c}" -o "${GRADER_NAME}.o" "${coloring_flag}"
		vecho "Removing grader source..."
		vrun rm "${grader_c}"
		files_to_compile+=("${GRADER_NAME}.o")
		vecho "Added grader object file to the list of files to compile."
	...
	exe_file="${PROBLEM_NAME}.exe"
	vecho "Compiling and linking..."
	vrun capture_compile gcc ${C_OPTS} "${files_to_compile[@]}" -o "${exe_file}" "${coloring_flag}"
	check_warning "${WARNING_TEXT_PATTERN_FOR_C}"
else
