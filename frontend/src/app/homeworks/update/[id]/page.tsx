...
// TODO: Refactor the form component to be reused between create and update pages
const CreateContestPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchContest = async () => {
      try {
        const res = await apiGet(`/api/contests/${id}`);
        if (res.ok) {
          const data = await res.json();
          setTitle(data.title);
          setStartTime(data.startTime);
          setSoftDeadline(data.endTime);
          setHardDeadline(data.submissionEndTime);
          setSelectedProblems(data.problems.map((p: Problem) => p.serialNumber));
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setModalMessage(`Homework updated successfully!`);
      } else {
        let msg = `Homework update failed (${res.status}). `;
        try {
          const data = await res.json();
  ...
  return (
    <div className="min-h-screen bg-background px-8 py-12">
      <div className="mx-auto max-w-3xl">
        <h1 className="mb-6 text-2xl font-bold text-slate-100">Update {title}</h1>
